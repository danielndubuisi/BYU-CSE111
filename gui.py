...
def populate_main_window(frm_main):
    ...
    Return: nothing
    """
    # Create a label that prompts user to "Enter radius"
    lbl_radius = Label(frm_main, text="Enter radius:")

    # Create a float entry box where the user will enter the radius.
    ent_radius = FloatEntry(frm_main, width=4, lower_bound=0, upper_bound=900000)

    # Create a label that displays value
    lbl_results_display = Label(frm_main, text="The Area of the circle is:")
    lbl_radius_units = Label(frm_main, text="millimeters")

    # Create labels that will display the results.
    lbl_area = Label(frm_main, width=30)
    lbl_area_units = Label(frm_main, text="millimeter sqrd")
    ...
    lbl_results_display.grid(     row=1, column=0, padx=1, pady=3)
