"""This program computes and outputs the molar mass and number of moles of a chemical compound. For additional features, the program tries to find the user entered chemical formula in a list of known formulas and if found prints the formula name to the user.
"""
from formula import parse_formula

def make_periodic_table():
    """Takes no parameters and creates and returns a compound dictionary. 
    ...
    "Al": ["Aluminum", 26.9815386],
        ⋮
    """
    # compound dict in format {symbol: [name, atomic mass], ...}
    periodic_table_dict = {
    "Ac": ["Actinium", 227],
    "Ag": ["Silver", 107.8682],
    "Al": ["Aluminum", 26.9815386],
...
# a dictionary with known chemical formulas and their names
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for inner_list in symbol_quantity_list:
        ...
        quantity = int(inner_list[QUANTITY_INDEX])

        # Get the atomic mass for the symbol from the dictionary.
        symbol_value = periodic_table_dict[symbol]
        atomic_mass = float(symbol_value[ATOMIC_MASS_INDEX])
...
def get_formula_name(formula, known_molecules_dict):
    """Try to find formula in the known_molecules_dict. If formula is in the known_molecules_dict, return
...
def main():
    ...
    parsed_formula_list = parse_formula(chemical_formula, periodic_table)

    # Call the compute_molar_mass function to compute the molar mass of the molecule from the compound list.
    molar_mass = compute_molar_mass(parsed_formula_list, periodic_table)
    ...
    # Call the get_formula_name function and print the compound name for the user to see
    compound_name = get_formula_name(chemical_formula, known_molecules_dict)
    print(f'Compond name - {compound_name}')
