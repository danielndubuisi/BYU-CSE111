...
# test water column height function
def test_water_column_height():
    assert water_column_height(0.0, 0.0) == approx(0.0, abs=0.01)
...
# test pressure gain from water height function
...
# test pressure loss from pipe function
...
# test pressure loss from fittings function
...
# test conversion from kPa to psi function
def test_convert_kPa_to_psi():
    assert convert_kPa_to_psi(0.00) == approx(0.000, abs=0.001)
    assert convert_kPa_to_psi(56.12) == approx(8.1395, abs=0.001)
    assert convert_kPa_to_psi(158.70) == approx(23.017, abs=0.001)
    assert convert_kPa_to_psi(-210.15) == approx(-30.4797, abs=0.001)
